...
class MlpBlock(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        x = nn.gelu(x)
        x = nn.LayerNorm(use_bias=False, use_scale=True)(x)  # normformer
        output = nn.Dense(
...
class DiTBlock(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        # Attention Residual.
        x_norm = nn.LayerNorm(use_bias=False, use_scale=True)(x)
        ...
        attn_x = nn.LayerNorm(use_bias=False, use_scale=True)(attn_x)  # normformer
        x = x + (gate_msa[:, None] * attn_x)

        # MLP Residual.
        x_norm2 = nn.LayerNorm(use_bias=False, use_scale=True)(x)
...
class FinalLayer(nn.Module):
    ...
    def __call__(self, x, c):
        ...
        x = modulate(nn.LayerNorm(use_bias=False, use_scale=True)(x), shift, scale)
        x = nn.Dense(
...
class DiT(nn.Module):
    ...
    def __call__(self, x, t, y, train=False, force_drop_ids=None):
        ...
        x = PatchEmbed(self.patch_size, self.hidden_size)(
            x
        ...
        # print("DiT: After scan, shape is", x.shape)

        x = FinalLayer(self.patch_size, out_channels, self.hidden_size)(
            x, c
        )  # (B, num_patches, p*p*c)
